...
exports.updateTrack = async (req, res, next) => {
	...
	try {
		await axios.post(process.env.CENTER_URL + "/api/updateContactTrackByCase", centralData)
	} catch (error) {
	}
	// send to central end //
	...
	if (old_status !== status) {
		...
		const htmlContents = await ejs.renderFile(path.join(__dirname, '../../views/email/emailupdate.ejs'), emailConfig)
...
exports.addMessage = async (req, res, next) => {
	...
	const message = decryptedData(req.body.message)
	const case_number = decryptedData(req.body.case_number)
	const patient_id = decryptedData(req.body.patient_id)
	const assign = decryptedData(req.body.assign)
	const person_type = decryptedData(req.body.person_type)
