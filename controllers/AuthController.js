...
exports.confirmSignUp = async (req, res, next) => {
	...
	if (req.session.ying === md5(md5(md5(captcha)))) { // correct captcha
		...
		if (patientInfo) { // already existed patient information
			...
			return res.status(200).json({ status: "success", case_number: case_number })
		}
	} else { // wrong captcha
		const captcha = await getCaptcha(6, 0, false)
		req.session.ying = captcha.key
...
exports.confirmHelp = async (req, res, next) => {
	...
	if (help === "account") { // forgot account
		...
		if (patientInfo) { // existed patient
			...
			const htmlContents = await ejs.renderFile(path.join(__dirname, '../views/email/helpemail.ejs'), emailConfig)
		...
		if (req.session.ying === md5(md5(md5(captcha)))) { // correct captcha
			...
			if (patientInfo) { // existed patient
				...
				const htmlContents = await ejs.renderFile(path.join(__dirname, '../views/email/passwordemail.ejs'), emailConfig)
